...
from wtforms import StringField,EmailField,PasswordField,URLField,SubmitField,IntegerField   #this is function 
...
class user_input(FlaskForm):  #need to inherit the class to use form
    ...
    username = StringField(label= "Enter the name :", validators= [DataRequired()])    #this use to take input username as name  (database)
...
class url_input(FlaskForm):  #in analyze page need to create object of this class to get url
    ...
    request_count = IntegerField(label = "enter the count of request", validators=[DataRequired()])
